...
本文先梳理registry的部署与使用，下一篇再整理harbor的部署与使用。
...
|-|-|
|10.13.13.201|仓库服务器|
|10.13.13.202|docker服务器|

## 服务端部署

### 修改 docker 可以通过 http 访问
需要设置 docker 可以通过 http 访问，有两种方法，二选一即可：
...
vim /usr/lib/systemd/system/docker.service
  ...
  #在ExecStart后边添加：--insecure-registry=10.13.13.201：5000
  ExecStart=/usr/bin/dockerd --insecure-registry=10.13.13.201:5000 -H fd:// --containerd=/run/containerd/containerd.sock
...
vim /etc/docker/daemon.json
  ...
  {
    "insecure-registries":["10.13.13.201:5000"]
  }
...
```shell
docker pull registry  #下载一个可用镜像
docker tag registry 10.13.13.201:5000/registry_test/registry:v1 #通过tag命令添加自定义标签
```

### 上传镜像
```shell
docker push 10.13.13.201:5000/registry_test/registry:v1
```
...
```shell
yum install jq -y
curl -s 'http//10.13.13.201:5000/v2/registry_test/registry/tags/list'
```

### 下载镜像
```shell
docker pull 10.13.13.201:5000/registry_test/registry:v1
```
...
### 准备删除脚本
```shell
vim /usr/sbin/delete_docker_registry_image
chmod +x /usr/sbin/delete_docker_registry_image
...
delete_docker_registry_image -i registry_test/registry:v1
```

## 附件
### 附件1. delete_docker_registry_image
```python
#!/usr/bin/env python
"""
Usage:
