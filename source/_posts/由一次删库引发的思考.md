...
deleteOperation () {
	#根据当前日期创建回收站
    mkdir -p ${trashDir}${dateSuffix}
	#将文件添加时间后缀并移动到回收站
    ls -d ${@} | while read deleteFiles
    do
        mv ${deleteFiles} ${trashDir}${dateSuffix}/${deleteFiles}_${dateTimeSuffix}
    done
}
```
...
####Record file
recordFile() {
    echo ${dateTimeSuffix} >> ${recordFile}
    `pwd` >> ${recordFile}
    echo ${@} >> ${recordFile}
}
```
...
deleteOperation ${@}
echo "Files have removed to ${trashDir}${dateSuffix}."
```
